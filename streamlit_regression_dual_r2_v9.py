...
def interpret_fit_quality(p_value, r2_fit_basis, fit_basis_label):
    if not np.isfinite(p_value) or not np.isfinite(r2_fit_basis):
        return (
            ...
            "The global model test or R² could not be computed from the current data and model settings.",
            "The p-value tests whether the fitted model improves over a flat no-effect model.",
            "R² shows how much variability is explained on the chosen fit basis.",
        )

    if p_value < 0.001:
        p_text = "very strong statistical evidence that the fitted model improves over a flat no-effect model"
    elif p_value < 0.05:
        p_text = "statistical evidence that the fitted model improves over a flat no-effect model"
    elif p_value < 0.10:
        p_text = "weak or borderline statistical evidence that the fitted model improves over a flat no-effect model"
    else:
        p_text = "no clear statistical evidence that the fitted model improves over a flat no-effect model"

    if r2_fit_basis < 0.10:
        r2_text = "very low explanatory power"
    elif r2_fit_basis < 0.30:
        r2_text = "low explanatory power"
    elif r2_fit_basis < 0.50:
        r2_text = "moderate explanatory power"
    elif r2_fit_basis < 0.75:
        r2_text = "good explanatory power"
    else:
        r2_text = "strong explanatory power"

    if p_value < 0.05 and r2_fit_basis < 0.20:
        headline = "Significant trend, but weak explanatory power."
        detail = (
            f"On the selected fit basis ({fit_basis_label}), the model detects a statistically significant trend, "
            "but it explains only a small part of the total variability. Treat the curve mainly as descriptive/interpolative."
        )
    elif p_value < 0.05 and r2_fit_basis < 0.50:
        headline = "Significant trend with modest explanatory power."
        detail = (
            f"On the selected fit basis ({fit_basis_label}), the model captures a real trend and explains a moderate fraction "
            "of the variability. The fit is useful, but predictions and X opt should still be interpreted with caution."
        )
    elif p_value < 0.05:
        headline = "Significant trend with strong explanatory power."
        detail = (
            f"On the selected fit basis ({fit_basis_label}), the model is statistically significant and explains a large fraction "
            "of the variability. This supports a relatively strong fitted relationship in the current dataset."
        )
    elif r2_fit_basis >= 0.50:
        headline = "Reasonable curve shape, but not statistically supported."
        detail = (
            f"On the selected fit basis ({fit_basis_label}), the curve follows the data reasonably well, but the global model test "
            "is not statistically significant. Avoid over-interpreting X opt unless supported by other analyses."
        )
    else:
        headline = "Weak support for the fitted relationship."
        detail = (
            f"On the selected fit basis ({fit_basis_label}), the model is not statistically significant and explains little variability. "
            "This fit should be treated as exploratory only."
        )

    p_explanation = f"p-value: {p_text}."
    r2_explanation = f"R²: {r2_text} on the selected fit basis."
        ...
        try:
            ...
            with res_col2:
                with st.container(border=True):
                    ...
                    st.write("- **p-value** is the global test of whether the fitted model improves over a flat no-effect model on the chosen fit basis.")
